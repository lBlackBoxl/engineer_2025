...
void chassis_task(void const *pvParameters)
{
        ...
        chassis_control_loop(&chassis);
				...
				{			
						...
						CAN_cmd_chassis_uwb(0);
						clamp_flag = 0;
						error_cnt = 0; 
						Half_rod_cnt = 0;
						chassis.move_mode = Wait;
						time_cnt = 0;
//						CAN_cmd_chassis_clamp(0);
				}
				else if(chassis.chassis_mode == RUN_MODE)
				{
						...
						CAN_cmd_chassis_uwb(chassis.motor_uwb.give_current);
...
static void chassis_init(all_key_t *chassis_key_init, chassis_t *chassis_init)
{
    ...
    const static fp32 chassis_y_order_filter[1] = {CHASSIS_ACCEL_Y_NUM};
		...
		last_rc_mid = chassis_init->chassis_RC->rc.s[RC_SW_MID];
		...
		PID_Init(&chassis_init->uwb_motor_speed_pid, UWB_SPEED_PID_MAX_OUT, UWB_SPEED_PID_MAX_KD,0.0f,
						UWB_SPEED_PID_KP, UWB_SPEED_PID_KI, UWB_SPEED_PID_KD,0.0f,0.0f,0.00106103295394596890512589175582f,0.0f,5,0x11);	
		...
		for (uint8_t i = 0; i < 4; i++)
    {
		...
		chassis_init->motor_uwb.motor_measure = get_chassis_motor_point(5);
//		//获取台阶达妙数据
...
//		key_init(&chassis_key_init->chassis_rotate_key,B);
		...
		key_init(&chassis_key_init->mode_change_key, CTRL);
		key_init(&chassis_key_init->Power_get_key, F);
		key_init(&chassis_key_init->clamp_key, Q);
		key_init(&chassis_key_init->arm_mode_change_key, E);
}
...
static void chassis_set_mode(all_key_t *chassis_set_key, chassis_t *chassis_set_mode)
{
		...
		if(last_s[RC_SW_RIGHT].itself.mode != last_s[RC_SW_RIGHT].itself.last_mode
			|| (all_key.mode_change_key.itself.flag == 1 && all_key.Power_get_key.itself.mode != all_key.Power_get_key.itself.last_mode))
		{
...
//		}
//		else if(chassis_set_mode->chassis_RC->rc.s[RC_SW_MID] == 1)
//		{
//			chassis_set_mode->arm_mode = ONE_KEY_MODE;
//		}
//		else if(chassis_set_mode->chassis_RC->rc.s[RC_SW_MID] == 2)
//		{
...
//		}
		...
		last_rc_mid = chassis_set_mode->chassis_RC->rc.s[RC_SW_MID];
...
//				clamp_mode = 0;
//		}
		if(chassis_set_mode->chassis_mode == NO_POWER_MODE)
		{
				clamp_mode = 0;
		}
		if(clamp_mode == 1)
		...
		if(uwb_mode == 1)
		{
				if(fabs(chassis_set_mode->motor_uwb.speed) < 1.0f)
						uwb_reset_count++;
				else
						uwb_reset_count = 0;
		}
		if(uwb_mode == 1 && uwb_reset_count >= 250)
		{
				chassis_set_mode->motor_uwb.round_cnt = 0;
				chassis_set_mode->motor_uwb.offset_ecd = chassis_set_mode->motor_clamp.motor_measure->ecd;
				uwb_mode = 2;
				uwb_reset_count = 0;
		}	
		...
		if((last_s[0].itself.mode != last_s[0].itself.last_mode) || ((all_key.mode_change_key.itself.flag == 0) && (all_key.gou_dong_key.itself.mode != all_key.gou_dong_key.itself.last_mode)))
		{
...
//		}
		...
		if(clamp_mode == 1)
		{
				chassis_set_mode->motor_clamp.speed_set =  15.0f;
		}
		else if(clamp_mode ==2)
		{
				...
				if(all_key.mode_change_key.itself.flag == 1 && all_key.clamp_key.itself.mode != all_key.clamp_key.itself.last_mode)
				{
					clamp_flag = 1 - clamp_flag;
				}
				if(clamp_flag == 1)
				{
						chassis_set_mode->motor_clamp.position_set = -0.12f;
				}
				else
				{
		...
		if(Ore_Pump_flag == 0)
		{
				HAL_GPIO_WritePin(Ore_Pump_1_GPIO_Port, Ore_Pump_1_Pin, GPIO_PIN_RESET);
				HAL_GPIO_WritePin(Ore_Pump_2_GPIO_Port, Ore_Pump_2_Pin, GPIO_PIN_RESET);
		}
		else if(Ore_Pump_flag == 1)
		{
				HAL_GPIO_WritePin(Ore_Pump_1_GPIO_Port, Ore_Pump_1_Pin, GPIO_PIN_SET);
				HAL_GPIO_WritePin(Ore_Pump_2_GPIO_Port, Ore_Pump_2_Pin, GPIO_PIN_RESET);
		}
		else if(Ore_Pump_flag == 2)
		{
				HAL_GPIO_WritePin(Ore_Pump_1_GPIO_Port, Ore_Pump_1_Pin, GPIO_PIN_SET);
				HAL_GPIO_WritePin(Ore_Pump_2_GPIO_Port, Ore_Pump_2_Pin, GPIO_PIN_SET);
		}
		
		//UWB
		if(uwb_mode == 2)
		{
				if(chassis_set_mode->move_mode == GouDong)
				{
					chassis_set_mode->motor_uwb.position_set = 0.05f;
				}
				else
				{
...
void chassis_key_check(all_key_t *chassis_key_check)
{
		...
		key_itself_press_num(&(chassis_key_check->chassis_rotate_key),2);
		key_itself_press_num(&(chassis_key_check->mode_change_key),2);
		key_itself_press_num(&(chassis_key_check->clamp_key), 2);
		key_itself_press_num(&(chassis_key_check->arm_mode_change_key), 2);
		key_itself_press_num(&(chassis_key_check->Power_get_key), 2);
...
static void chassis_feedback_update(chassis_t *chassis_update)
{
    ...
    chassis_update->motor_clamp.speed = chassis_update->motor_clamp.motor_measure->speed_rpm * RPM_TO_RAD_S * REDUCTION_RATIO_2006;	
		...
		//UWB
		chassis_update->motor_uwb.position = (chassis_update->motor_uwb.round_cnt * ECD_RANGE + chassis_update->motor_uwb.motor_measure->ecd - chassis_update->motor_uwb.offset_ecd) * MOTOR_ECD_TO_ANGLE_2006;
    chassis_update->motor_uwb.speed = chassis_update->motor_uwb.motor_measure->speed_rpm * RPM_TO_RAD_S * REDUCTION_RATIO_2006;	
...
//		}
    ...
    if (chassis_control == NULL || all_key.mode_change_key.itself.flag == 1)
    {
        return;
    }
...
static void chassis_control_loop(chassis_t *chassis_control_loop)
{
    ...
    for (i = 0; i < 4; i++)
    {
		...
		else if(clamp_mode == 1)
		{
				...
				chassis_control_loop->motor_clamp.give_current = int16_constrain((int16_t)chassis_control_loop->clamp_motor_speed_pid.Output,-10000,10000);
		}
		else if(clamp_mode == 2)
		{
		...
		if(uwb_mode == 0)
		{
			chassis_control_loop->motor_uwb.give_current = 0;
		}
		else if(uwb_mode == 1)
		{
		...
		{
				PID_Calculate(&chassis_control_loop->uwb_motor_position_pid, chassis_control_loop->motor_uwb.position, chassis_control_loop->motor_uwb.position_set);
				chassis_control_loop->motor_uwb.speed_set = chassis_control_loop->uwb_motor_position_pid.Output;
				PID_Calculate(&chassis_control_loop->uwb_motor_speed_pid, chassis_control_loop->motor_uwb.speed, chassis_control_loop->motor_uwb.speed_set);	
