...
typedef struct
{
	...
	fp32 one_key_position[6];
	fp32 repostion_position[6];
	fp32 three_ore_position[6];
	fp32 pre_Au_reposition[6];
	fp32 Au_reposition[6];
	fp32 Ag_reposition[6];
...
typedef struct
{
	...
	float     Joint_Final[4];     //四组解中选择出来的最终解
	float     high;
