...
void chassis_task(void const *pvParameters)
{
        ...
        chassis_control_loop(&chassis);
				...
				else if(chassis.chassis_mode == RUN_MODE || chassis.chassis_mode == CLAMP_MODE)
				{
...
static void chassis_set_mode(all_key_t *chassis_set_key, chassis_t *chassis_set_mode)
{
		...
		{
			chassis_set_mode->chassis_mode = CLAMP_MODE;
		}
...
//			DWT_Delay(0.0003f);
		...
		//跑路模式||爬台阶模式
		else if(chassis_control->chassis_mode == RUN_MODE || chassis_control->chassis_mode == CLAMP_MODE)
		{
			CAN_cmd_4310_enable(DM_4310_M1_TX_ID,hcan2);
			DWT_Delay(0.0003f);
			CAN_cmd_4310_enable(DM_YAW_TX_ID,hcan2);
			DWT_Delay(0.0003f);
			if(chassis_control->chassis_mode == CLAMP_MODE)
			{
